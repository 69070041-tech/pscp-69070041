...
```text
input:
บรรทัดแรกรับค่าเดือนเป็นจำนวนเต็ม
บรรทัดสองรับค่าวันที่เป็นจำนวนเต็ม

output:
ชื่อฤดูการต่างๆตามโจทย์สั่ง(spring,summer,fall,winter)

constaints:
ฤดูจะเปลี่ยนทุกๆวันที่ 21 ของเดือนที่หารด้วย 3 ลงตัว และวันกับเดือนต้องไม่เป็น 0 หรือติดลบ
```
...
```text
Step 1:รับค่าวันและเดือนเข้ามา

Step 2:เช็คว่าเดือนอยู่ในชุดตัวเลข (3,6,9,12) นี้หรือไม่ 

             Step 2.1:ถ้าใช้ให้เช็คต่อว่าวันที่ว่ามันมากกว่าหรือเท่ากับ 21 หรือไม่

                              Step 2.11:ถ้าใช้ให้เช็คเดือนต่อว่าเป็นเดือนไหนใน ชุดตัวเลข (3,6,9,12) ถ้าตรงกับเดือนไหนให้ตอบตามฤดูนั้นและแสดงผลออกมา(คำตอบจะไม่ตรงกับ2.12เพราะในคำสั่งบอกว่าจะเปลี่ยนฤดูทุกวันที่21ของเดือนที่หาร3ลงตัว)

             Step 2.2:ถ้าไม่ให้เช็คเดือนต่อว่าเป็นเดือนไหนใน ชุดตัวเลข (3,6,9,12) ถ้าตรงกับเดือนไหนให้ตอบตามฤดูนั้นและแสดงผลออกมา

Step 3:ถ้าเช็คแล้วไม่อยู่ในชุดตัวเลข (3,6,9,12) ให้แสดงผลฤดูตามเดือนต่างๆก่อนเดือนที่จะถึงเลขนั้นที่จะหารด้วย3ลงตัว
```
...
```text
ใช้วิธีเดิมคือรับค่าวันกับเดือน แล้วเช็คเดือนว่าอยูในชุดเลข 3,6,9,12 ถ้าอยุ่เช็ควันว่ามากกว่าหรือเท่ากับ 21 มั้ย ถ้าใช้เช็คเดือนแล้วตอบตามฤดู แต่ถ้าน้อยกว่า 21 ให้แสดงผลฤดูตามเดือน ถ้ามันไม่อยู่ในตัวเลขให้ แสดงผลฤดูก่อนเดือนที่เลขหาร 3 ลงตัว
```
...
```text
เช็คการทำงานของเงื่อนไขเปลี่ยนฤดูทุกวันที่ 21ของเดือนที่หาร3ลงตัวว่าจะเปลี่ยนหรือไม่ 
```
