...
```text
Input:
ให้โปรแกรมรับข้อมูลตัวเลขจำนวนเต็มบวกใส่ในตัวแปล N

Output:
แสดงจำนวนเต็มบวกตั้ง N ถึง 0 ที่หารด้วยเลข 10 ลงตัว

Constraints:
ควรพิจารณาเลขที่น้อยกว่า 0 เป็นต้นไปด้วย เพราะ เลขใดๆที่ modulo ด้วย เลขจำนวนจริงไม่ว่าจะเป็นบวกหรือลบก็ตาม ก็จะมีคำตอบเหมือนกัน
```
...
```text
Step 1: อ่าน input ที่ได้จากการรับค่า N
Step 2: ตรวจสอบว่า N มีค่ามากกว่าหรือเท่ากับ 0 หรือไม่ ถ้าใช่ให้ทำตามเงื่อนไขต่อไป ถ้าไม่ให้ทำการไม่แสดงผล
Step 3: ตรวจสอบว่า N modulo 10 ลงตัวหรือไม่ ถ้าใช้ให้ัแสดงผลออกมาตามเลขที่หารลงตัว หาก Modulo 10 ไม่ลงตัว ให้ทำการลบค่านั้นลง 1 
Step 4: ทำงานต่อไปเรื่อยจนกว่าจะเงื่อนไขใน Step 2 จะไม่เป็นจริง
```
...
```text
วิธีสุดท้ายของฉันเหมือนกับแผนแรก ฉันตรวจสอบว่า N มีค่าน้อยกว่าหรือเท่ากับ 0 หรือไม่ เพื่อตรวจสอบ N เป็นไปตามเกณฑ์ไการทำงานหรือไม่ จากนั้นใช้ Modulo เพื่อตรวจสอบว่า N หารเอาเศษนั้นลงตัวหรือไม่ ถ้าใช่ให้แสดงผลตัวเลขนั้น ถ้าไม่ให้ลบเลขนั้นเรื่อยๆจนกว่าจะหารเอาเศษด้วย 10 จะลงตัวอีกรอบ ทำงานจนกว่าจะไม่ตรงเงื่อนไขว่า N มีค่าน้อยกว่าหรือเท่ากับ 0 หรือไม่
```
...
```text
เพื่อทดสอบว่าจะแสดงผลเลข 0 หรือไม่(เลข 0 หารไม่เอาเศษด้วย 10 ลงตัวหรือไม่)
```
...
```text
ตามโครงสร้างเงื่อนไขใน while n >= 0: มันจะต้องไม่แสดงผลข้อมูลออกมาเนื่องจากไม่ตรงเงื่อนไข
```
...
```text
ทดสอบว่าการรับค่าปกติมั้ยโดยถ้าใส่เป็น string เข้าไปมันต้องบอกว่าค่าไม่ถูกต้อง
```
